...
cd('..\')
currentpath=pwd;
path=[currentpath,'\experimentos\experimento3'];
save(path);
cd(mainpath)
formatoPoint='.fig';
formato='fig';
...
title('Tensión máxima y mínima alcanzada en el experimento 3')
xlabel('Nodos')
ylabel('Tensiones(pu)')
grid on
hold off
...
title('Cargas máxima y mínima alcanzada en el experimento 3)')
xlabel('Linea')
ylabel('Carga')
grid on
hold off
...
title('Carga en la linea 1 (experimento 3)')
xlabel('Horas (anual)')
ylabel('Carga')
grid on
hold off
...
%axis(ejeslimites)
legend('P(kW)','Q(kVAr)')
title('Potencias en el centro de transformación (experimento 3)')
xlabel('Horas (anual))')
ylabel('Potencia')
grid on
hold off
