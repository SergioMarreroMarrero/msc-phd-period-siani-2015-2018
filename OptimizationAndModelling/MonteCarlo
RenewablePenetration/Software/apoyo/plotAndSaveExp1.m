...
cd('..\')
currentpath=pwd;
path=[currentpath,'\experimentos\experimento1'];
save(path);
cd(mainpath)
formatoPoint='.fig';
formato='fig';
...
title('Tensión máxima y mínima alcanzada en el experimento 1')
xlabel('Nodos')
ylabel('Tensiones(pu)')
grid on
hold off
...
title('Cargas máxima y mínima alcanzada en el experimento 1)')
xlabel('Linea')
ylabel('Carga')
grid on
hold off
...
title('Tensiones en las cargas 1,35,24 a lo largo de un año (Experimento 1).')
xlabel('Horas(anual)')
ylabel('Tensiones(pu)')
grid on
hold off
...
title('Carga en la linea 1 (experimento 1)')
xlabel('Horas (anual)')
ylabel('Carga')
grid on
hold off
...
energiaActivaDiaria=mean(potenciasActivaSum)*24;
energiaReactivaDiaria=mean(potenciasReactivaSum)*24;
...
%axis(ejeslimites)
legend('P(kW)','Q(kVAr)')
title('Potencias en el centro de transformación (experimento 1)')
xlabel('Horas (anual))')
ylabel('Potencia')
grid on
hold off
...
title('Evolucion de las tensiones e intensidades maximas en el algoritmo de descenso (experimento 1)')
xlabel('Iteracion')
ylabel('V,I (pu)')
grid on
hold off
